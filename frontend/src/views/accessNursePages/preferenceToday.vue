...
<template>
    <div class="container">
        <h3 style="margin-top: 20px">Shift Preference Today</h3>
...
<!--        :max-height="tableHeight"-->
        <el-table ref="table"
                  border
                  :default-sort = "{prop:'pressure_icon_ID', order:'descending'}"
                  :data="shiftTable"
...
<script>
    export default {
        ...
        methods: {
            //compose the url for requestion
            geturl(str1,str2){
                return (this.siteConfig.baseApi+'/api/preferences?$sort[shift_date]=-1&shift_date[$gt]='+str1+'&shift_date[$lt]='+str2);
            },
            async renderTable(){
                //record the time
                ...
                let endTimeUTC = startTimeUTC + 86390000
                let finalurl = this.geturl(startTimeUTC,endTimeUTC)

                //send requestion to server
                //this part request all preferences for today
